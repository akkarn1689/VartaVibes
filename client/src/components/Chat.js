...
const Chat = () => {
  ...
  useEffect(() => {
    if (selectedUserId) {
      // fetchMessages();
      fetch('http://localhost:8080/messages/' + selectedUserId, {
        method: 'GET',
        headers: {
          Accept: "application/json",
          "Content-Type": "application/json"
  ...
  const sendNewMessage = (e) => {
    ...
    setNewMessage("");
    setMessages(prev => ([...prev, { text: newMessage, sender: userData._id, receiver: selectedUserId, time: new Date().toLocaleString() }]));
  };
